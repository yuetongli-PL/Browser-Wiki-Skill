...
const NATIVE_SEED_HEADER_OPTIONS = Object.freeze({
  allowCookieHeader: false,
});
...
function mergeHeaders(...sources) {
  ...
  return normalizeDownloadResourceConsumerHeaders(result, NATIVE_SEED_HEADER_OPTIONS);
}
...
function hasSensitiveSeedText(value) {
  return /authorization\s*:|cookie\s*:|set-cookie\s*:|bearer\s+|(?:access|refresh|auth|csrf|xsec)?_?token=|csrf(?:_token)?=|xsrf=|sessdata=|session(?:id)?=|synthetic-[^\s"'<>]*(?:auth|cookie|csrf|session|token)/iu
    .test(String(value ?? ''));
}

function firstArtifactSafeText(...values) {
  for (const value of values) {
    const normalized = normalizeText(value);
    if (normalized && !hasSensitiveSeedText(normalized)) {
      return normalized;
    }
  }
  return '';
...
function flattenSeedValue(value, inherited = {}, baseUrl = '') {
  ...
  const nextInherited = {
    ...inherited,
    title: firstArtifactSafeText(value.title, value.resolvedTitle, value.name, inherited.title),
    sourceUrl: firstArtifactSafeText(value.sourceUrl, value.requestedUrl, value.pageUrl, value.finalUrl, inherited.sourceUrl),
    referer: firstArtifactSafeText(value.referer, value.sourceUrl, value.requestedUrl, value.pageUrl, value.finalUrl, inherited.referer),
    headers: mergeHeaders(inherited.headers, value.headers),
    groupId: firstArtifactSafeText(value.groupId, value.noteId, value.videoId, value.id, inherited.groupId),
  };
...
function fileNameForSeed(seed, title, mediaType, url, index) {
  const explicit = firstArtifactSafeText(seed.fileName, seed.filename);
  if (explicit) {
    return explicit;
  }
  const ext = extensionForResource(seed, url, mediaType);
  const prefix = String(index + 1).padStart(4, '0');
  return `${prefix}-${compactTitle(title, firstArtifactSafeText(seed.id) || 'download')}.${ext}`;
}
...
export function resolveNativeResourceSeeds(siteKey, plan, sessionLease = null, context = {}, options = {}) {
  ...
  const inherited = {
    title: firstArtifactSafeText(request.title, request.resolvedTitle, plan.source?.title),
    sourceUrl: firstArtifactSafeText(request.sourceUrl, request.inputUrl, request.url, request.input, plan.source?.canonicalUrl, plan.source?.input),
    referer: firstArtifactSafeText(request.referer, request.sourceUrl, request.inputUrl, request.url, request.input, plan.source?.canonicalUrl, plan.source?.input),
    headers: mergeHeaders(normalizeSessionLeaseConsumerHeaders(sessionLease), request.headers, request.downloadHeaders),
  };
  ...
  const resources = seedEntries
    .map((seed, index) => {
      ...
      const title = firstArtifactSafeText(seed.title, seed.resolvedTitle, seed.name, request.title, plan.source?.title, seed.id);
      const mediaType = inferMediaType(seed, options.defaultMediaType);
      return normalizeDownloadResource({
        id: firstArtifactSafeText(seed.id, seed.assetId, seed.videoId, seed.noteId) || undefined,
        url,
        method: seed.method,
        ...
        mediaType,
        sourceUrl,
        referer: firstArtifactSafeText(seed.referer, sourceUrl, inherited.referer) || undefined,
        ...
        groupId: firstArtifactSafeText(seed.groupId, seed.noteId, seed.videoId, request.groupId, request.title, plan.id) || undefined,
        metadata: {
          ...(isObject(seed.metadata) ? seed.metadata : {}),
          siteResolver: siteKey,
          title: title || undefined,
          sourceTitle: firstArtifactSafeText(request.title, plan.source?.title) || undefined,
        },
      }, index, NATIVE_SEED_HEADER_OPTIONS);
    })
    .filter(Boolean);
  ...
  }, plan, NATIVE_SEED_HEADER_OPTIONS);
}
