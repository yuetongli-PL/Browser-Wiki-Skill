...
const BATCH1_AV_SITES = Object.freeze([
  {
    host: 'rookie-av.jp',
    siteKey: 'rookie-av',
    adapterId: 'rookie-av',
    baseUrl: 'https://rookie-av.jp/',
    repoSkillDir: 'skills/rookie-av',
    detailUrl: 'https://rookie-av.jp/works/detail/RKI-001/',
    searchUrl: 'https://rookie-av.jp/search/list?keyword=sample',
    categoryUrl: 'https://rookie-av.jp/works/list',
    expectedTitle: 'Rookie Sample Work',
    expectedWorkId: 'RKI-001',
    expectedActor: 'Rookie Actor',
  },
  {
    host: 'madonna-av.com',
    siteKey: 'madonna-av',
    adapterId: 'madonna-av',
    baseUrl: 'https://madonna-av.com/',
    repoSkillDir: 'skills/madonna-av',
    detailUrl: 'https://madonna-av.com/works/detail/MAD-001/',
    searchUrl: 'https://madonna-av.com/search/list?keyword=sample',
    categoryUrl: 'https://madonna-av.com/works/list',
    expectedTitle: 'Madonna Sample Work',
    expectedWorkId: 'MAD-001',
    expectedActor: 'Madonna Actor',
  },
  {
    host: 'dahlia-av.jp',
    siteKey: 'dahlia-av',
    ...
    repoSkillDir: 'skills/dahlia-av',
    detailUrl: 'https://dahlia-av.jp/works/detail/DAH-001/',
    searchUrl: 'https://dahlia-av.jp/search/list?keyword=sample',
    categoryUrl: 'https://dahlia-av.jp/works/list',
    expectedTitle: 'Dahlia Sample Work',
    expectedWorkId: 'DAH-001',
    expectedActor: 'Dahlia Actor',
  },
  {
    host: 'www.sod.co.jp',
    siteKey: 'sod',
    adapterId: 'sod',
    baseUrl: 'https://www.sod.co.jp/',
    repoSkillDir: 'skills/sod',
    detailUrl: 'https://www.sod.co.jp/works/detail/SOD-001/',
    searchUrl: 'https://www.sod.co.jp/search/list?keyword=sample',
    categoryUrl: 'https://www.sod.co.jp/works/list',
    expectedTitle: 'SOD Sample Work',
    expectedWorkId: 'SOD-001',
    expectedActor: 'SOD Actor',
  },
]);
...
test('Batch 1 AV sites are registered with capabilities and valid profiles', async () => {
  const registry = await readSiteRegistry(WORKSPACE_ROOT);
  const capabilities = await readSiteCapabilities(WORKSPACE_ROOT);

  for (const site of BATCH1_AV_SITES) {
    const registryRecord = registry.sites[site.host];
    const capabilityRecord = capabilities.sites[site.host];

    assert.ok(registryRecord, `${site.host} must exist in config/site-registry.json`);
    assert.ok(capabilityRecord, `${site.host} must exist in config/site-capabilities.json`);
    ...
    assert.equal(registryRecord.downloadEntrypoint, undefined, `${site.host} must remain navigation-only in Batch 1`);
...
test('Batch 1 AV URL normalization stays inside each allowed host family', async () => {
  for (const site of BATCH1_AV_SITES) {
    ...
    assert.equal(typeof adapter.normalizeUrlForNavigation, 'function');
    assert.equal(typeof adapter.isAllowedUrlFamily, 'function');
    assert.equal(
    ...
    assert.equal(
      adapter.isAllowedUrlFamily({ inputUrl: 'https://example.invalid/works/detail/RKI-001/', profile: resolved.profile }),
      false,
    );
  }
});

test('Batch 1 AV adapter exposes catalog navigation capability for each site', async () => {
  for (const site of BATCH1_AV_SITES) {
    const resolved = await resolveSite({
      workspaceRoot: WORKSPACE_ROOT,
    ...
    assert.equal(typeof adapter.extractContentDetailSemantics, 'function');
...
test('Batch 1 AV fixture detail parsing emits semantic metadata without sensitive material', async () => {
  for (const site of BATCH1_AV_SITES) {
    const resolved = await resolveSite({
      workspaceRoot: WORKSPACE_ROOT,
      inputUrl: site.detailUrl,
    ...
    const parsed = resolved.adapter.extractContentDetailSemantics({
      ...
      profile: resolved.profile,
      headers: {
        cookie: 'SESSDATA=synthetic-batch1-cookie',
        authorization: 'Bearer synthetic-batch1-token',
      },
    });
...
test('Batch 1 AV adapter returns reasonCode on unsupported or cross-family detail parsing', async () => {
  for (const site of BATCH1_AV_SITES) {
    const resolved = await resolveSite({
      workspaceRoot: WORKSPACE_ROOT,
      inputUrl: site.detailUrl,
    ...
    const result = resolved.adapter.extractContentDetailSemantics({
      siteKey: site.siteKey,
      host: site.host,
      finalUrl: 'https://example.invalid/works/detail/RKI-001/',
      pageType: 'book-detail-page',
      title: 'Wrong Host',
...
test('Batch 1 AV onboarding discovery emits complete tri-state artifacts and passes coverage gate', async () => {
  for (const site of BATCH1_AV_SITES) {
    const resolved = await resolveSite({
      workspaceRoot: WORKSPACE_ROOT,
...
test('Batch 1 AV skills lint as instruction-only public navigation skills', async () => {
  for (const site of BATCH1_AV_SITES) {
