---
name: attackers
description: Instruction-only Skill for the verified https://attackers.net/ URL family. Use when Codex needs read-only public navigation, search, list pagination, or detail-page metadata extraction for ATTACKERS official catalog pages.
---
...
- `https://attackers.net/`
- `https://www.attackers.net/`
- Same-host subpaths only.

External marketplace, streaming, CDN, affiliate, mirror, fan, reseller, short-link, social, and advertisement domains are outside this skill.

## Supported workflows

- Open public landing, age gate, works list, search, performer, series, label/category, and work detail pages.
- Search by work code, title, performer, series, visible category/tag, release date, or keyword.
...
- Downloading, recording, resolving player URLs, scraping CDN assets, removing DRM, or mirroring samples.
...
- Extracting credentials, cookies, local storage, CSRF values, authorization headers, session ids, browser profile data, tokens, or account state.
- Expanding titles, categories, or page text into graphic or invented descriptions.

## Safe browsing rules

- Stay on `attackers.net` or `www.attackers.net`.
- Use read-only page loads, ordinary links, public search, visible filters, and public pagination.
...
- Stop and report when the site requires login, payment, blocked access, suspicious redirect, or unsupported interaction.
...
Do not capture cookies, tokens, profile paths, request headers, local storage, account identifiers, CSRF values, authorization material, raw media/player URLs, manifests, or media segment URLs.

## Pagination, search, and detail rules

- Search: use public search UI or query URL; record query text and final URL.
- Listing traversal: follow visible next or numbered controls; stop on duplicates, repeated page content, missing next link, or requested limit.
- Detail verification: open work pages for final work code, title, dates, performer names, maker, label, runtime, and tags.
...
- No native downloader is claimed for `attackers`.
...
- Never write cookies, account data, browser profile state, tokens, authorization headers, CSRF values, local storage, or media stream URLs.
- Keep descriptions neutral and technical; do not add graphic detail beyond source titles.

## Verification notes

- Confirm generated files are limited to `skills/attackers/`.
...
- `outside-url-family`: link leaves `attackers.net`.
...
- "Extract ATTACKERS public work metadata from this detail URL."
- "Search ATTACKERS for this performer and list official work links with dates."
- "Build a neutral CSV of the latest ATTACKERS listing page."

## Refused or downgraded request examples

- "Download the movie from ATTACKERS." Downgrade to public metadata and report `download-unsupported`.
