---
name: maxing
description: Instruction-only Skill for the verified https://www.maxing.jp/ URL family. Use when Codex needs read-only public navigation, search, list pagination, or detail-page metadata extraction for MAXING official catalog pages.
---
...
- Downloading, recording, stream URL extraction, embedded-player reverse engineering, CDN scraping, DRM removal, or sample/media mirroring.
...
- Expanding page titles, tags, or summaries into graphic, speculative, or invented descriptions.

## Safe browsing rules

- Stay on `maxing.jp` or `www.maxing.jp` public pages.
...
- No native downloader is claimed for `maxing`.
...
- Maxing encoding/public-entry retrieval issues are reportable boundary evidence. Record the visible encoding or public-entry state and stop; do not bypass it or promote it into API/downloader support.
...
- Confirm generated files are limited to `skills/maxing/`.
...
- `outside-url-family`: navigation leaves `maxing.jp`.
...
- `encoding-public-entry-boundary`: public entry retrieval or character encoding prevents reliable visible-page extraction and must be reported without bypass.
...
- "Search MAXING for this work code and return the official public metadata."
- "List latest MAXING public works with title, release date, performers, and source URL."
- "Open this MAXING detail page and extract neutral catalog fields."

## Refused or downgraded request examples

- "Download the MAXING video." Downgrade to official metadata and report `download-unsupported`.
